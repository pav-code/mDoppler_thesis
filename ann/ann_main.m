...
%  Skeleton provided by Stanford University, homework assignment.
% Change: sDir. First for train data (ln 32), and test data (ln 149)
...
if strcmp(sTrained,'true')
    ...
    for iFile=1:size(sFile,1)
      if regexp(sFile(iFile).name,'W_')
          aFileTag(iCount) = 1;
          iCount = iCount + 1;
     elseif regexp(sFile(iFile).name,'R_')
         aFileTag(iCount) = 2;      
         iCount = iCount + 1;          
      elseif regexp(sFile(iFile).name,'B_')
          aFileTag(iCount) = 3;      
          iCount = iCount + 1;          
      end
    end
...
for i = 1:(weight_grid*weight_grid)
    ...
    imagesc(reshape((Theta1(i,2:end)),[input_X,input_Y]))
    axis off
end

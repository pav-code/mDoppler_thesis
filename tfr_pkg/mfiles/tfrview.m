function [im1] = tfrview(tfr,sig,t,method,param,p1,p2,p3,p4,p5);
...
if istfraff(method),
 ...
 if istfr1(method) % Pav-add
   freq=(0.5*(-Nf2:Nf2-1)/Nf2); % display both positive and negative
 else
   freq=(0.5*(0:Nf2-1)/Nf2); % display only the positive frequencies
 end
end
%Pav-Add change this to proper labeling!
...
%   freqs=linspace(f1,f2,Nf2);
 ...
 if (tcol==1),
  ...
  elseif display==2,              % imagesc
   if linlogtfr==0,
    clear im1
    im1 = imagesc(ts,freqr,tfr); axis('xy');
   else
    clear im1   
    %imagesc(ts,freqr*1000,log10(tfr));axis('xy'); %pav-add check
    %im1 = imagesc(ts,freqr*2776,log10(tfr));axis('xy'); %pav-add check
    im1 = imagesc(ts,freqr,log10(tfr));axis('xy'); %pav-add check
   end
   %set(axtfr,'Ylim', [fmin*fs fmax*fs]);
   DisplayStr=', imagesc';
  ...
  elseif display==3,              % pcolor
   if linlogtfr==0,
    im1 = pcolor(ts,freqr,tfr); shading interp;
   else
    im1 = pcolor(ts,freqr,log10(tfr)); shading interp;
   end
   set(axtfr,'Ylim', [fmin*fs fmax*fs]);
   DisplayStr=', pcolor';

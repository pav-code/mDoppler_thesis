function [im1] = tfrqview(tfr,sig,t,method,p1,p2,p3,p4,p5);
...
if istfraff(method),
 ...
 if istfr1(method) % Pav-add
   freq=(0.5*(-Nf2:Nf2-1)/Nf2); 
 else
...
if exist('options.mat'),
 ...
 threshold=0.1;  % visualization threshold
 linlogtfr=1;    % tfr visualization scale : 0 for linear 1 for logarithmic
...
while choice~=ChoiceClose,                       % while not close
 if RefreshFigure & CallTfrView,                 % Call to tfrview
  ...
  param = [display, linlog, threshold, levelnumb, Nf2, layout,...
           fs, isgrid, fmin, fmax];
  % All branches of the if have been fucked up: "[im1] = "
  if (nargin<=4),
   [im1] = tfrview(tfr,sig,t,method,param);
  elseif (nargin==5),
   [im1] = tfrview(tfr,sig,t,method,param,p1);
  elseif (nargin==6),
   [im1] = tfrview(tfr,sig,t,method,param,p1,p2);
  elseif (nargin==7),
   [im1] = tfrview(tfr,sig,t,method,param,p1,p2,p3);
  elseif (nargin==8),
   [im1] = tfrview(tfr,sig,t,method,param,p1,p2,p3,p4);
  elseif (nargin==9),
   [im1] = tfrview(tfr,sig,t,method,param,p1,p2,p3,p4,p5);
  end;
 end;
 ...
 % Main menu
 if (strcmp(method, 'TYPE1') || strcmp(method, 'TFRSCALO'))
     choice = ChoiceClose;
 else
  ...
  elseif (display==6),
   if MatlabVersion>=5,
    ...
    colmap=menu('COLOR MAP :',...
                'hsv','jet','cool','bone','gray','hot','prism',...
                'brighten','darken','permute','spin','cancel');
    if     colmap== 2,  colormap(hsv(levelnumb));
    elseif colmap== 1,  colormap(jet(levelnumb));
    elseif colmap== 3,  colormap(cool(levelnumb));
    elseif colmap== 4,  colormap(bone(levelnumb));
    elseif colmap== 5,  colormap(gray(levelnumb));
    elseif colmap== 6,  colormap(hot(levelnumb));

function [tfr,t,f, im1] = tfrspwv(x,t,N,g,h,trace);
...
% Pav-Add: nonelegant approach: based on number of output arguments
%  we do an fftshift for the negative frequencies. Abetter way would have 
%  been to add another output argument and decide based on that if the sign
%  al is 'analytic'.
if (nargout==0),
 tfrqview(tfr,x,t,'tfrspwv',g,h);
elseif (nargout==4),
 tfr1 = fftshift(tfr,1); %(Pav-add)positive and negative frequency alignment   
 %[im1] = tfrqview(tfr,x,t,'tfrspwv',g,h);
 [im1] = tfrqview(tfr1,x,t,'TYPE1',g,h);
elseif (nargout==3),
 f=(0.5*(0:N-1)/N)';
end;
end
